...
class SearchBar extends Component {
	...
	handleSubmit = (event) => {
		...
		API.getGeoData(this.state.trailSearch)
			.then(({ data }) => {
				// console.log(data);
				API.getTrails(data.features[0].geometry.coordinates[1], data.features[0].geometry.coordinates[0])
					.then((res) => {
						this.setState({
